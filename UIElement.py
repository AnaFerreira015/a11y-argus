...
class UIElement:
    ...
    def __init__(self, id: str, content: str, bounds: Tuple[int, int, int, int], node=None):
        ...
        """
        self.id = id
        self.content = content or ""  # Tratar texto nulo como string vazia
        self.bounds = bounds
        self.node = node
...
def check_overlapping_elements(elements: List[UIElement], xml_root, tolerance: int = 0) -> List[Tuple[UIElement, UIElement]]:
    """
    Verifica se há elementos de texto que se sobrepõem, considerando um tolerance
    que simula um aumento nos espaçamentos conforme o critério 1.4.12 (Text Spacing).
    Elementos com relação hierárquica (pai-filho) são ignorados.
    """
    overlapping_pairs = []
    for i, elem1 in enumerate(elements):
        for j, elem2 in enumerate(elements):
            if i >= j:
                continue  # Evita comparar o mesmo elemento ou pares repetidos
...
def is_overlapping(bounds1: Tuple[int, int, int, int], bounds2: Tuple[int, int, int, int], tolerance: int = 0) -> bool:
    ...
    # Se os elementos se tocam exatamente, considere que NÃO há sobreposição.
    if x1_max == x2_min or x2_max == x1_min or y1_max == y2_min or y2_max == y1_min:
        return False

    # Agora, aplique o tolerance se desejado.
    return not (
...
def is_parent_or_child(elem1: UIElement, elem2: UIElement) -> bool:
    # Se qualquer um dos nós for ancestral do outro, eles têm relação pai–filho.
...
def check_duplicate_text(elements: List[UIElement], xml_root) -> List[UIElement]:
    """Verifica se há textos duplicados, considerando hierarquia e contexto."""
    seen_content = {}
    duplicate_elements = []
    for element in elements:
        if not element.content.strip():  # Ignorar textos vazios ou apenas com espaços
            continue
        # Contexto adicional: inclua o pai no identificador
...
def get_parent_id(element_id: str, xml_root) -> str:
    """Obtém o ID do pai de um elemento."""
